...
const Voluntarios = () => {
    ...
    const fetchNormas = async () => {
        try {
            const response = await axios.get(`${_api}/api/voluntario/getAll`);
    ...
    const handleFormEditSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`${_api}/api/voluntario/update`, {
                id: eventoModal.id,
    ...
    const handleFormSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post(`${_api}/api/voluntario/create`, {
                idUser: e.target[0].value,
            }).then(response => {
                console.log(response.data);
                fetchNormas();
        ...
        <>
            <EventsContainer>
                <div className="events__header">
                    <h2>Voluntarios</h2>
                <button onClick={() => setIsOpen(!isOpen)}>Agregar nuevo voluntario</button>
                </div>
                <table>
                    <thead>
                        <tr>
            ...
            <Modal isOpen={isOpenDeleteModal}>
                <div>Borrar Voluntario</div>
                <p>Desea borrar al voluntario?</p>
                <div>
                    <button onClick={async () => {
                        try {
                            console.log(eventoModal);
                            await axios.delete(`${_api}/api/voluntario/remove/${eventoModal.id}`).then(response => {
                                console.log(response.data);
                                fetchNormas();
